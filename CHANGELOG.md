...
- Moderation primitives: inbox now drops deliveries from domains on the peers block-list (after signature verification) with 403. New admin endpoints `GET /api/moderation/flags`, `GET|POST /api/moderation/blocks` guarded by the same `TRAILFED_OUTBOX_SECRET` bearer token. Inbound `Flag` activities are persisted into `activities` for moderator review.
- User registration endpoint (`POST /api/register`) creates a local actor with an Argon2id-hashed password and a fresh RSA keypair. Validates username shape, password length, and `(username, domain)` uniqueness. `actors.password_hash` added via migration `0001`.
